...
from mechanize import Browser # used for webscrapping
import favicon 
...
def home(request):
    if request.method == 'POST':
        if "signup-form" in request.POST:
            ...
            msg = f"{request.user}. You are logged out."
            logout(request)
            messages.success(request, msg)
            return HttpResponseRedirect(request.path)
            ...
            if new_login is None:
                ...
                msg = f"Login failed!, Make sure you are using the right account."
                messages.error(request, msg)
                return HttpResponseRedirect(request.path)
            else:
                ...
                global_code = code
                send_mail(
                    "Django Password Manager: confirm your email CodewithDancan",
    ...
    if request.user.is_authenticated:
        passwords = Password.objects.all().filter(name =request.user)
